...
{
    m_pDbOperate = new Storage::DbOperate();
...
Storage* DbOperator::MakeMemOperate()
{
    if (m_pDbMemRequest == NULL)
    {
        m_pDbMemRequest = new Storage();
    }
    else
    {
                ...
                bool bGroupBy, bool bOrderBy, const std::string& strOrder)
{
    ...
    if (bOrderBy)
    {
        Storage::DbOperate::OrderBy *pOrderBy = m_pDbOperate->add_orderby_col();
        pOrderBy->set_col_name(strFieldName);
        if (std::string("DESC") == strOrder || std::string("desc") == strOrder)
        {
            pOrderBy->set_relation(Storage::DbOperate::OrderBy::DESC);
        }
        else
        {
            pOrderBy->set_relation(Storage::DbOperate::OrderBy::ASC);
        }
    }
    return(true);
}
...
bool DbOperator::AddCondition(Storage::DbOperate::Condition::E_RELATION eRelation,
                const std::string& strFieldName, const std::string& strFieldValue,
                E_COL_TYPE eFieldType, const std::string& strRightFieldName)
{
    Storage::DbOperate::ConditionGroup* pConditionGroup;
    Storage::DbOperate::Condition* pCondition;
    ...
    pConditionGroup->set_relation(Storage::DbOperate::ConditionGroup::AND);
...
bool DbOperator::AddCondition(int iGroupIdx,
                Storage::DbOperate::ConditionGroup::E_RELATION eGroupRelation,
                Storage::DbOperate::Condition::E_RELATION eRelation,
                const std::string& strFieldName, const std::string& strFieldValue,
                E_COL_TYPE eFieldType, const std::string& strRightFieldName)
{
    if (iGroupIdx >= m_pDbOperate->conditions_size())
    {
        Storage::DbOperate::ConditionGroup* pConditionGroup = m_pDbOperate->add_conditions();
        pConditionGroup->set_relation(eGroupRelation);
        Storage::DbOperate::Condition* pCondition = pConditionGroup->add_condition();
        pCondition->set_relation(eRelation);
        pCondition->set_col_name(strFieldName);
        pCondition->set_col_type(eFieldType);
        if (strRightFieldName.length() > 0)
    ...
    {
        Storage::DbOperate::Condition* pCondition = m_pDbOperate->mutable_conditions(iGroupIdx)->add_condition();
        pCondition->set_relation(eRelation);
        pCondition->set_col_name(strFieldName);
        pCondition->set_col_type(eFieldType);
        if (strRightFieldName.length() > 0)
...
void DbOperator::SetConditionGroupRelation(Storage::DbOperate::ConditionGroup::E_RELATION eRelation)
{
