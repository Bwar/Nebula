        ...
        const MsgBody& oInMsgBody)
{
    ...
    if (oConfigInfo.ParseFromString(oInMsgBody.data()))
    {
        ...
        if (fout.good())
        {
            ...
            m_pSessionManager->SendToWorker(CMD_REQ_SET_CUSTOM_CONFIG, GetSequence(), oInMsgBody);
            m_pSessionManager->SendToWorker(CMD_REQ_RELOAD_CUSTOM_CONFIG, GetSequence(), oInMsgBody);
